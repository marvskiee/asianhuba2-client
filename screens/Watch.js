...
const Watch = (props) => {
  ...
  const final_item = props?.route?.params?.item;
  const load = async (id) => {
    let count = 0;
    await axios_config
      .get("info?id=" + id)
      .then(async (res) => {
        ...
        setPlayer({
          ...
          episode_number: res.data?.episodes[0]?.episode,
        });
  ...
  const watchHandler = async (item) => {
    setPlayer({ video: null });
    setWatchData(null);
    await axios_config
      .get("watch?episodeId=" + item?.id)
      .then((res) => {
        setWatchData(res.data);
        setPlayer({
          image: final_item?.image,
  ...
  const WatchHeader = () => {
    return (
      <View style={{ backgroundColor: colors.black }}>
        ...
        <Text style={[styles.label, { paddingHorizontal: 10 }]}>Servers</Text>
        <View
          style={{
            padding: 10,
            flexDirection: "row",
